...
void workerThread() {
    ...
    Aligned a;
    unsigned long long rofs;
    unsigned long long wofs;
    ...
    cout << std::this_thread::get_id() <<" thread stopped read/writes." << endl;
    while(1) /* avoid terminating the thread, noop time*/
    {
        sleepsecs(5);
    }
}
...
void go() {
    ...
    const char* fquit = "./mongoperf__end";
    ...
    if(!(options["create"].trueValue())) {
        lf = new LogFile(fname, true); /* points to FILE_BEGIN */
        cout << " Skipping new file creation...using " << fname << endl;
    }
    else {
    ...
    unsigned t = 0;
    while (1) {
        if (t++ % 60 == 0 && boost::filesystem::exists(fquit))
        {
            stop = true;
            cout << "Stopping threads... ";
            try {
                boost::filesystem::remove(fquit);
            } catch (...) {
                cout << "error deleting file " << fquit << endl;
                return;
            }
            sleepsecs(10); // allow for threads to finish their read/write ops
            cout <<"Done." <<endl;
            break;
        }
    ...
    try {
        ...
        if (argc > 1) {
            cout <<
                ...
                "    create:<bool>     // new file creation (default true)\n" 
                "(default 0/never)\n"
                "  }\n"
                "\n"
